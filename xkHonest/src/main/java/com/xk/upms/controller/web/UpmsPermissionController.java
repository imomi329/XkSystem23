...
//    @RequiresPermissions("upms:permission:read")
    ...
    public String index(UpmsPermissionReq resources, Model model) {
        if (resources.getSystemId() == null) {
            resources.setSystemId((long) 1); // 預設 upms權限管理系統
        }

        this.info(model, this.getClass().getAnnotation(RequestMapping.class).value()[0]);
        model.addAttribute("page_list", upmsPermissionService.listBy(resources));
        ...
        model.addAttribute("fragmentPackage", "manage/f_permission");
