...
//    @RequiresPermissions("upms:role:read")
    ...
    public String index(Model model) {
        ...
        model.addAttribute("fragmentPackage", "role");
        return ADMIN_INDEX;
    }
    ...
    public String view(@PathVariable("id") Long id, Model model) {
        this.info(model, this.getClass().getAnnotation(RequestMapping.class).value()[0]);
        model.addAttribute("page_list", upmsRoleService.list(null));
        model.addAttribute("entity", upmsRoleService.selectByPrimaryKey(id));
        model.addAttribute("users", upmsUserService.findAllUsersByRoleId(id));
        model.addAttribute("system", "upms");
        model.addAttribute("fragmentPackage", "role/detail");
